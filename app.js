// RAGAS v6.1.0 application UI logic — extracted for maintainability
...
async function load(){if(loadPromise)return loadPromise;loadPromise=(async()=>{try{const payDate=$('payableDate')?.value||today;let allTimeReports;[products,cash,sales,salesReturns,purchases,collections,supplierPayments,allTimeReports,alerts,payableOptions,salesConfig,outlets,outletStock]=await Promise.all([api('/products'+(showArchived?'?includeArchived=1':'')),api('/cash'),api('/sales'),api('/sales-returns'),api('/purchases'),api('/collections'),api('/supplier-payments'),api(`/reports?from=${ALL_REPORT_FROM}&to=${ALL_REPORT_TO}`),api('/alerts'),api(`/reports/detail?type=payables&to=${encodeURIComponent(payDate)}`),api('/sales/config'),api('/outlets'),api(`/inventory/outlet-stock?to=${ALL_REPORT_TO}`)]);dashboardReportCache=allTimeReports;if(!reportCache)reportCache=allTimeReports;payableOptions=payableOptions.report||{suppliers:[],purchases:[],payments:[]};outlets=Array.isArray(outlets)?outlets:(Array.isArray(outlets?.rows)?outlets.rows:Array.isArray(outlets?.outlets)?outlets.outlets:[]);outletStock=Array.isArray(outletStock)?outletStock:(Array.isArray(outletStock?.rows)?outletStock.rows:Array.isArray(outletStock?.data)?outletStock.data:[]);payableOptionsDate=payDate;render();renderDashboard();updateCreditDiscountUI();renderAlerts();loadSpoilageQueue().catch(()=>{});loadAnalytics(true).catch(()=>{});setSync('Synced');}catch(e){setSync('Offline / API error',true);throw e}finally{loadPromise=null}})();return loadPromise}
...
load=async function(){
  ...
  loadPromise=(async()=>{try{
    const payDate=$('payableDate')?.value||today;let arDetail,apDetail;
    [products,cash,sales,salesReturns,purchases,collections,supplierPayments,alerts,arDetail,apDetail,salesConfig,outlets,outletStock]=await Promise.all([
      api('/products'+(showArchived?'?includeArchived=1':'')),api('/cash'),api('/sales'),api('/sales-returns'),api('/purchases'),api('/collections'),api('/supplier-payments'),api('/alerts'),api(`/reports/detail?type=receivables&to=${encodeURIComponent(payDate)}`),api(`/reports/detail?type=payables&to=${encodeURIComponent(payDate)}`),api('/sales/config'),api('/outlets'),api(`/inventory/outlet-stock?to=${ALL_REPORT_TO}`)
    ]);
    operationalReceivables=arDetail.report||{total:0,customers:[],sales:[],unallocated:[]};payableOptions=apDetail.report||{total:0,suppliers:[],purchases:[],payments:[]};payableOptionsDate=payDate;dashboardReportCache=null;reportCache=null;
